...
class App:
    def __init__(self, config_path: str = 'config.json', mode: str = 'paper', dry_run: bool = False):
        ...
        # Core services
        self.storage = DataStorage(db_path=self.config['paths']['db_path'], data_path=self.config['paths']['data_path'])
        self.data_provider = DataProvider(self.storage)
        self.gemini = GeminiAgent(api_key=self.config['api_keys'].get('gemini_api_key'))
        self.notifier = DingTalkNotifier(token=self.config['api_keys'].get('dingtalk_token'))
        self.trader = TradeExecutor(account_id=self.config['account']['qmt_account_id'], mini_qmt_path=self.config['account']['mini_qmt_path'], max_drawdown=self.config['strategy'].get('max_drawdown', 0.2))
        self.scheduler = Scheduler()
        # Order manager
        from src.trading.order_manager import OrderManager
        self.order_manager = OrderManager(self.trader, max_retries=self.config.get('system', {}).get('order_max_retries', 3), retry_delay=self.config.get('system', {}).get('order_retry_delay', 2), paper_mode=self.dry_run or (self.mode!='live'))

        # Risk manager
        from src.risk.risk_manager import RiskManager
        self.risk_manager = RiskManager(self.trader, storage=self.storage, max_drawdown=self.config.get('strategy', {}).get('max_drawdown', 0.2), max_position_pct=self.config.get('strategy', {}).get('max_position_pct', 0.1), max_position_per_symbol=self.config.get('strategy', {}).get('max_position_per_symbol', 100000))
    ...
    def build_strategy(self):
        ...
        if not name:
            self.logger.warning("No active strategy configured; using TopGainerStrategy by default")
            from src.strategies.top_gainer_strategy import TopGainerStrategy
            self.strategy = TopGainerStrategy("TopGainer_v1", self.config['strategy'], self.gemini)
        ...
        try:
            ...
            for attr in dir(mod):
                if attr.lower().endswith('strategy') and name.lower() in attr.lower():
                    ...
                    self.strategy = cls(name, self.config['strategy'], self.gemini) if callable(cls) else None
                    break

        except Exception:
            self.logger.warning(f"Could not import configured strategy module for '{name}', falling back to TopGainerStrategy")
    ...
    def schedule_paper_run(self, minutes: int = 60, rounds: int = 1, stocks: list = None, report_path: str = 'reports/paper_run.json'):
        """Schedule a periodic paper-run job using the internal Scheduler."""
        from scripts.paper_run import run_paper

        def job():
            try:
                self.logger.info(f"Scheduled paper run starting: rounds={rounds}, stocks={stocks}")
                run_paper(config_path=self.config_path, rounds=rounds, stocks=stocks or self.config.get('strategy', {}).get('stocks_pool', []), report_path=report_path)
    ...
    def start_trader_if_needed(self):
        ...
        if started:
            ...
            self.logger.warning("Trader failed to connect; running in observation mode.")
        return started
