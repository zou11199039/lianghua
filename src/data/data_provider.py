...
class DataProvider:
    ...
    def download_data(self, stock_list, period="1d", start_time="", end_time=""):
        ...
        for code in stock_list:
            ...
            if not data_dict:
                logger.warning(
                    "xtdata.get_market_data_ex returned no data for %s", code
                )
                continue
            ...
            if code in data_dict and not data_dict[code].empty:
                ...
                try:
                    inserted = self.storage.save_market_data(
                        data_dict[code], code, period
                    )
    ...
    def get_kline(self, stock_list, period="1d", use_cache=True):
        ...
        if missing_stocks:
            ...
            data_dict = xtdata.get_market_data_ex(
                field_list=[], stock_list=missing_stocks, period=period, count=-1
            )
