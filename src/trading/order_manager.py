    ...
    def submit_signal(self, signal: dict) -> Optional[str]:
        ...
        if self.paper_mode:
            ...
            if getattr(self, 'risk_manager', None) and oid:
                self.risk_manager.apply_trade_effect(signal.get('code'), int(signal.get('volume', 0)))
            # mark pending as simulated filled
            self.pending_orders[oid] = {'signal': signal, 'attempts': 0, 'status': 'filled'}
            return oid
        ...
        while attempt <= self.max_retries:
            try:
                ...
                if oid == 'RISK_HALT' or not oid:
                    logger.warning(f"Order rejected by risk manager or empty id on attempt {attempt}: {oid}")
                    return 'RISK_HALT'
                logger.info(f"Order placed: {oid} (attempt {attempt})")
                # record pending order and attempts
                self.pending_orders[oid] = {'signal': signal, 'attempts': attempt, 'status': 'submitted'}
                return oid
    ...
    def handle_receipt(self, order_id: str, status: str, info=None):
        ...
        if status == 'filled' or status == 'partial':
            ...
            if getattr(self, 'risk_manager', None):
                self.risk_manager.apply_trade_effect(sig.get('code'), int(sig.get('volume', 0)))
            rec['status'] = 'filled'
        ...
        if status in ('rejected', 'failed'):
            ...
            if rec['attempts'] <= self.max_retries:
                logger.info(f"Retrying failed order {order_id}, attempt {rec['attempts']}")
                # resubmit
