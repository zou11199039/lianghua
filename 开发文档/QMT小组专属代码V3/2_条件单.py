...
run_end_time = datetime.datetime.strptime(
    (datetime.datetime.now().strftime("%Y-%m-%d") + f" 15:30"), "%Y-%m-%d %H:%M"
)
...
while True:
    for index, value in enumerate(order_list):
        ...
        code = value["symbol"]
        record_log(
            f'监测{code},条件:{value["fun"]},参数:{value["para"]},交易方向:{value["side"]}'
        )
        # 动态导入对应的py文件
        # __import__ 动态导入模块。详情请见：https://www.runoob.com/python/python-func-__import__.html
        cls = __import__(
            "conditions.%s" % value["fun"], fromlist=("",)
        )  # fromlist如果为空，导入的是factors包，所以需要给factors传入参数
        # 读取数据
        mkt_df = getattr(cls, "read_data")(xtdata, code, start_time=start_time)
        # 计算交易的信号
        signal = getattr(cls, "signal")(
            mkt_df, value["para"], xt_trader=xt_trader, user=user, code=code
        )
        # 删除数据，清理内存占用
        del mkt_df
        gc.collect()
        ...
        if (signal == 1) and (value["side"] == "buy"):
            ...
            buy_stock(
                ...
                code,
                order_volume,
                strategy_name=f'{code}{value["fun"]}买入',
            )
            # 下单后删除对应的数据
