...
def sell_stock(xt_trader, user, code, sell_volume, price=0, **kwargs):
    ...
    for _ in range(5):
        try:
            ...
            order_volume = (
                pos_res.can_use_volume
                if pos_res.can_use_volume < order_volume
                else order_volume
            )
            # 如果处理后的卖出股数为0,直接退出程序
            if order_volume == 0:
...
def subscription_data(xtdata, code_list, period, count=1000, **kwargs):
    ...
    for code in code_list:
        ...
        xtdata.download_history_data(
            code, period="tick", start_time=start_time, end_time=end_time
        )
...
def get_market_data(xtdata, code, period, before_open=False, drop_0=True):
    ...
    mkt_data = pd.DataFrame(
        xtdata.get_market_data(
            [], [code], period="tick", start_time=start, end_time=end
        )[code]
    )
    rename_dict = {
        "time": "时间",
    ...
    if period != "tick":
        if period == "open2now":
            ...
            mkt_data = (
                mkt_data.resample(rule=period, on="时间")
                .agg(resample_dict)
