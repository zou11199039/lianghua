...
logfile = os.path.join(
    LOG_DIR, f"download_history_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
)
logging.basicConfig(
    level=logging.INFO,
...
def parse_args():
    ...
    p.add_argument(
        ...
        type=str,
        required=True,
        help="逗号分隔的股票代码列表，如 600000.SH,000001.SZ",
    )
...
def main():
    ...
    if not qmt.ensure_running():
        logger.warning(
            "未检测到 QMT 终端运行，数据下载可能失败。请手动启动‘中航证券QMT实盘-交易端’并重试。"
        )

    logger.info(f"开始下载 {len(codes)} 支股票的历史数据...")
    try:
        provider.download_data(
            codes, period=args.period, start_time=args.start, end_time=args.end
        )
        logger.info("下载完成。")
