...
def run_paper(config_path='config.json', rounds=1, stocks=None, report_path=None, app=None, report_format='json'):
    ...
    for r in range(rounds):
        ...
        try:
            ...
            app.logger.debug('Data provider download skipped in paper run (may be mocked in tests)')
        ...
        app.logger.info(f"Paper run iter {r+1}: signals={len(signals)} executed={executed} pnl={simulated_pnl}")
    ...
    if report_path:
        ...
        if fmt == 'jsonl':
            ...
            import csv
            keys = ['timestamp','iteration','signals','executed','simulated_pnl','details_count']
            csv_path = report_path if report_path.endswith('.csv') else report_path + '.csv'
...
if __name__ == '__main__':
    ...
    res = run_paper(config_path=args.config, rounds=args.rounds, stocks=args.stocks, report_path=args.report)
