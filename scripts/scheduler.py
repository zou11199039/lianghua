...
if not os.path.exists(LOG_DIR):
    os.makedirs(LOG_DIR)
logfile = os.path.join(LOG_DIR, f"scheduler_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log")
...
def job_sync(codes, period, qmt_shortcut=None):
    try:
        ...
        if not qmt.ensure_running():
            logger.warning('QMT terminal not running, job will attempt to proceed (may fail)')
...
def main():
    p = ArgumentParser()
    p.add_argument('--interval', type=int, default=60, help='Interval in seconds for testing')
    p.add_argument('--codes', type=str, default='', help='Comma-separated codes to override config')
    ...
    codes = [c.strip() for c in (args.codes or cfg.get('watch_list', '600000.SH')).split(',') if c.strip()]
    ...
    sched.add_job(lambda: job_sync(codes, period, qmt_shortcut=shortcut), 'interval', seconds=args.interval, id='data_sync')
